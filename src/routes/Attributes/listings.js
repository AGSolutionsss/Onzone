  ...
  getData = () => {
    ...
    axios({
      ...
      .then((res) => {
        ...
        let response = res.data.attr;
        let tempRows = [];
        for (let i = 0; i < response.length; i++) {
          
            tempRows.push([
              i + 1,
              <img src={"https://houseofonzone.com/admin/storage/app/public/Attributes/"+response[i]["fabric_type_images"]} style={{width:'40px',height:'40px'}}/>,
              response[i]["fabric_type_category"],
              response[i]["fabric_type_types"],
              response[i]["fabric_type_status"],
              response[i]["id"],
            ]);
