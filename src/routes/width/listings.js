...
export default class NewListWidth extends React.Component {
  state = {
    loader: true,
    users: [],
    fabricInwardData: [],
  ...
  getData = () => {
    ...
    axios({
      ...
      .then((res) => {
        ...
        let response = res.data.width;
        let tempRows = [];
        for (let i = 0; i < response.length; i++) {
          
            tempRows.push([
              i + 1,
              response[i]["width_mea"],
              response[i]["width_status"], 
              response[i]["id"],
            ]);
