...
export default function View(props){
    ...
    useEffect(() => {
        ...
        axios({
            url: baseURL+"/fetch-work-order-received-view-by-id/" + id,
          method: "GET",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("login")}`,
          },
        }).then((res) => {
            setWorkOrder(res.data.workorderrc);
            setWorkOrderSub(res.data.workorderrcsub);
            ...
            {!loader && 
                <>
                    <div className="invoice-wrapper">
                        <PageTitleBar title="Work Order Receive" match={props.match} />
                        <div className="row">
                                        ...
                                        <div className="justify-content-between" style={{marginTop:'1cm',marginLeft: '1cm', marginRight: '1cm',marginBottom: '1cm', fontSize: '16px' }}>
                                            <table style={{width:'100%'}}>
                                                <tr>
                                                    ...
                                                    <td><span>:</span></td>
                                                    <td><span style={tablelabel}>{workorder.work_order_rc_factory}</span></td>
                                                    <td><span>Work Order No</span></td>
                                                    <td><span>:</span></td>
                                                    <td><span style={tablelabel}>{workorder.work_order_rc_id}</span></td>
                                                    <td><span>Date</span></td>
                                                    <td><span>:</span></td>
                                                    <td><span style={tablelabel}>{Moment(workorder.work_order_rc_date).format('DD-MM-YYYY')}</span></td>
                                                </tr>
                                                <tr>
                                                    <td><span>Brand</span></td>
                                                    <td><span>:</span></td>
                                                    <td><span style={tablelabel}>{workorder.work_order_rc_brand}</span></td>
                                                    <td><span>DC No</span></td>
                                                    <td><span>:</span></td>
                                                    <td><span style={tablelabel}>{workorder.work_order_rc_dc_no}</span></td>
                                                    <td><span>DC Date</span></td>
                                                    <td><span>:</span></td>
                                                    <td><span style={tablelabel}>{Moment(workorder.work_order_rc_dc_date).format('DD-MM-YYYY')}</span></td>
                                                </tr>
                                                <tr>
                                                    <td><span>No of Box</span></td>
                                                    <td><span>:</span></td>
                                                    <td><span style={tablelabel}>{workorder.work_order_rc_box}</span></td>
                                                    <td><span>Total Pcs</span></td>
                                                    <td><span>:</span></td>
                                                    <td><span style={tablelabel}>{workorder.work_order_rc_pcs}</span></td>
                                                    <td><span>Received By</span></td>
                                                    <td><span>:</span></td>
                                                    <td><span style={tablelabel}>{workorder.work_order_rc_received_by}</span></td>
                                                </tr>
                                                <tr>
                                                    <td><span>Remarks</span></td>
                                                    <td><span>:</span></td>
                                                    <td colSpan="7"><span style={tablelabel}>{workorder.work_order_rc_remarks}</span></td>
                                                </tr>
                                            </table>
                                            <hr/>
                                            <table style={{width:'100%'}}>
